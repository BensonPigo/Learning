P18 包裝掃描作業系統 - 高層級流程flowchart TD
...
P18 箱號掃描作業流程 (TxtScanCartonSP_Validating)flowchart TD
...
P18 載入箱號資料流程 (LoadDatas)flowchart TD
...
P18 載入掃描明細流程 (LoadScanDetail)flowchart TD
...
P18 條碼掃描處理流程 (ProcessBarcodeScan)flowchart TD
...
P18 查找或分配 Barcode 流程 (FindOrAssignBarcode)flowchart TD
...
P18 手動條碼掃描處理 (ProcessManualScan)flowchart TD
...
P18 處理手動掃描結果 (HandleManualScanResult)flowchart TD
...
P18 處理掃描完成邏輯 (HandleScanCompletion)flowchart TD
    ...
    style CheckEventType fill:#FFD700
P18 完成掃描後處理 (AfterCompleteScanCarton)flowchart TD
...
P18 RFID 掃描主流程 (HandleRFIDScanResult)flowchart TD
    ...
    style Start fill:#90EE90
    style End fill:#90EE90
P18 處理單一 RFID 標籤 (ProcessSingleTag)flowchart TD
...
P18 處理 LLL RFID 標籤 (ProcessLLLTag)flowchart TD
...
P18 處理 NIKE RFID 標籤 (ProcessNikeTag)flowchart TD
    ...
    style CheckUPC fill:#FFD700
P18 處理 RFID 掃描結果 (HandleScanResults)flowchart TD
...
P18 顯示 RFID 錯誤訊息 (ShowErrorMessages)flowchart TD
...
P18 校正檢查機制 (alert_Calibration)flowchart TD
...
P18 缺件關閉處理 (LackingClose)flowchart TD
...
P18 更新缺件狀態 (UpdateLackingStatus)flowchart TD
